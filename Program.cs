	...
	internal class Program
	{
		...
		private static void GameLoop()
		{
			...
			do
			{
                ...
                else if (userInput == "S" || userInput == "D" || userInput == "B" || userInput == "C" || userInput == "P")
                {
					ReplaceShip(_player1Map, userInput);
                }
                else
                {
            ...
            do
            {
                ...
                {
                    ReplaceShip(_player2Map, userInput);
                }
                else
                {
                    Console.WriteLine("Invalid input. Please try again.");
		...
		{
            // Find the ship on the board and remove it
            int sizeCounter = 0;
            for (int i = 0; i < _mapSize; i++)
			{
            ...
            bool validPlacement = CheckShipPlacement(map, row, col, sizeCounter, direction); // Check if the ship can be placed

            if (validPlacement)
            {
                placeShipOnBoard(map, row, col, sizeCounter, direction, ship.ToString()); // Place the ship on the board
